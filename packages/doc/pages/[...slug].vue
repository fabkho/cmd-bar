...
onMounted(async () => {
  const res = await fetch('https://api.github.com/repos/fabkho/cmd-bar')
...
<template>
  <div>
          ...
          >
            <HeadlessDialogPanel class="relative flex flex-1 w-full max-w-xs mr-16 bg-black">
              ...
              <nav class="m-5 space-y-6 text-sm">
                <NuxtLink to="/" class="mb-5 text-2xl font-semibold"> CmdBar </NuxtLink>
                <section v-for="(item, index) in navigation" :key="index">
                  <NuxtLink
                    ...
                    :class="
                      item._path === router.currentRoute.value.path
                        ? 'text-blue-400'
                        : 'text-neutral-500'
                    "
                  >
                  ...
                  <ul class="space-y-3">
                    <li v-for="(c, _index) in item.children" :key="_index">
                      <NuxtLink
                        :to="c._path"
                        class="hover:underline"
                        :class="
                          c._path === router.currentRoute.value.path
                            ? 'text-blue-400'
                            : 'text-neutral-500'
                        "
                      >
                ...
                <section>
                  ...
                  <ul class="space-y-3">
                    <li>
                      <a
                        target="_blank"
                        href="https://github.com/fabkho/cmd-bar"
    ...
    <header class="flex items-center justify-between p-2 bg-transparent md:hidden">
      <NuxtLink to="/" class="text-2xl font-semibold"> CmdBar </NuxtLink>
    ...
    <div class="max-w-5xl mx-auto mt-20">
      ...
      <aside class="flex-col hidden ml-4 md:fixed md:flex">
        ...
        <nav class="space-y-4 text-sm">
          ...
          <section>
            <h2 class="mt-4 mb-2">Links</h2>
            <ul class="space-y-1">
              <li>
                <a
                  target="_blank"
                  href="https://github.com/Teygeta/nuxt-doc"
                  class="hover:underline text-neutral-500"
                >
                  GitHub ({{ githubStars }})
                </a>
